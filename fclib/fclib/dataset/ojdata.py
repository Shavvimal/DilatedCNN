...
from utils import git_repo_path
from feature_utils import df_from_cartesian_product
...
def download_ojdata(dest_dir="."):
    ...
    if not data_exists:
        ...
        try:
            ...
            print(f"rda Path: {rda_path}")
            print(f"Script Path : {script_path}")
            output = subprocess.run(
                ["Rscript", script_path, rda_path, dest_dir], stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True
            )
            print(output.stdout)
